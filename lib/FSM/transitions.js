...
export const TRANSITIONS = {
  ...
  attackOnCollisionOrDie: 'attackOnCollisionOrDie',
...
const transitions = {
  stopAttackingWhenResultIdle: {
    characterUpdate: {
      basicAttack: (fsm, event) => {
        if (event.aggressor === fsm.character.id &&
  ...
  fleeOnSight: {
    characterUpdate: {
      '*': (fsm, event) => {
        if (event.character !== fsm.character.id && event.action === ACTIONS.WALKING &&
          vectorDistanceToVector(fsm.character.position, event.position) <= fsm.character.getViewRange()) {
  ...
  idleAfterCollision: {
    characterUpdate: {
      ...
      basicAttack: (fsm, event) => {
        if (event.character === fsm.character.id && event.result === 'collision'
          && (!event.collidedWith && event.collidedWith !== fsm.state.target)) {
          return buildState(ACTIONS.IDLE, { owner: fsm.state.owner });
        }
        return null;
      },
  ...
  dieAfterAttack: {
    characterUpdate: {
      basicAttack: (fsm, event) => {
        if (event.aggressor === fsm.character.id && event.action === ACTIONS.BASIC_ATTACK) {
          return buildState(ACTIONS.DIE, { owner: fsm.state.owner });
        }
        return null;
      },
