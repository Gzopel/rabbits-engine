...
export class CharacterFSM {
  ...
  handleEvent = (eventData) => {
    if ((this.transitions['*'] && !this.transitions['*'][this.state.action] ) ||
        !this.transitions[eventData.event] ||
       (!this.transitions[eventData.event]['*'] &&
        !this.transitions[eventData.event][this.state.action])) {
      return;
    }
    const transition = this.transitions[eventData.event] ? this.transitions[eventData.event]: this.transitions['*'];
    const next = transition[this.state.action] ?
      transition[this.state.action](this, eventData) :
      transition['*'](this, eventData);
