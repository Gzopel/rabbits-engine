...
const calculateFirstCollisionPosition = (character, nextPosition, collidables) => {
  ...
  for (const other of collidables.values()) {
    if (character.collidesWith(other)) {
      ...
      if (other.position) { // this is an ugly hack for players that haven't spawned yet.
        ...
        if (d < r) {
          ...
          // This angle stuff is to only considering the collision if other is in the way
          if ((angle < minAngle || angle > maxAngle) && (!collision || d < collisionDistance)) {
