...
export const distanceToVector = (p, v, w) => {
  /*
  * This is buggy, does calculate distance to vector but distance to the line function defined by the vector.
  * Since the line function goes to infinity doesn't work as we would spect for distanceToVector.
  * */
...
export const angleFromVectors = (v, w) => {
  ...
  const division = dot / normProduct;
  const n = division > 1 || division < -1 ? division % 1 : division;
  return Math.acos(n);
};