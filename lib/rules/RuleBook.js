...
export class RuleBook {
  ...
  spawn = (state) => {
    for ( const location of this.map.spawnLocations ){
      let tries = 0;
      const diameter = location.r * 2; // just a magic number, move along
      while (tries < diameter) {
        const position = {
          x: location.x + Math.ceil(diameter * (Math.random() - 0.5)),
          z: location.z + Math.ceil(diameter * (Math.random() - 0.5)),
        };
