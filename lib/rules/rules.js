...
const characterArmour = (character) => {
  let armour = character.sheet.abilities.physical.stamina;
  Object.keys(character.items).forEach((itemKey) => {
...
const doAttack = (characterOne, characterTwo) => {
  ...
  const hitSuccess = rollDice(hitDices, weapon.dificulty).total - rollDice(dodgeDices, 7).total;
  if (hitSuccess < 1) {
    return [{
  ...
  const weaponDamage = weaponDamage(characterOne);
...
export const walk = (character, position) => {
...
export const attack = (characterOne, characterTwo) => {
  ...
  return walk(characterOne, characterTwo.position);
}
