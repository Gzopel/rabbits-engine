...
const doAttack = (characterOne, characterTwo) => {
  ...
  const hitSuccess = hitRoll - rollDice(dodgeDices, 7).total;
  if (hitSuccess < 1) {
    return [{
      character: characterTwo.id,
      result: 'dodge',
  ...
  const effectiveDamage = Math.min(0, damageRoll - armourRoll);
...
export class RuleBook {
  ...
  basicAttack = (state) => {
    ...
    console.log("basic attack ranges",weaponRange,"distance",distance)
